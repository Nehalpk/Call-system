...
#         )
#         if not response.ok:
#             print ("\n\n\n\n\n\n\n\n","This is eleven lab api key ",self.api_key)
...
class ElevenLabsSynthesizer(BaseSynthesizer[ElevenLabsSynthesizerConfig]):
    def __init__(
        ...
        # self.api_key="4a63a54a467321b66dfe09a58e895640"
        self.voice_id = synthesizer_config.voice_id or ADAM_VOICE_ID
        self.stability = synthesizer_config.stability
        self.similarity_boost = synthesizer_config.similarity_boost
        self.model_id = synthesizer_config.model_id
    ...
    async def experimental_streaming_output_generator(
        ...
        try:
            print ("this is the api from elevenlab ",self.api_key)
    ...
    async def create_speech(
        ...
        if self.stability is not None and self.similarity_boost is not None:
            voice_settings = self.elevenlabs.VoiceSettings(
                stability=self.stability, similarity_boost=self.similarity_boost
            )
            voice = self.elevenlabs.Voice(voice_id=self.voice_id, settings=voice_settings
            )
        else:
            voice = self.elevenlabs.Voice(voice_id=self.voice_id)
        #voice = self.elevenlabs.Voice(voice_id=self.voice_id)
        ...
        print ("Your eleven lab api key is: ", self.api_key)
        if not response.ok:
            print ("Your eleven lab api key is: ", self.api_key)
            raise Exception(f"ElevenLabs API returned {response.status} status code")
        if self.experimental_streaming:
