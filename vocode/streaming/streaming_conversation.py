...
from mongo_trans import MongoDBManager
mongo_manager = MongoDBManager("test","transcription","mongodb+srv://faisalqazi31:QWcJl6ybdwSKLwvp@ai-lead-generation-clus.dvybt5d.mongodb.net/?retryWrites=true&w=majority&appName=ai-lead-generation-cluster0")
# load_dotenv(override=True)
dialogues = {}

def add_to_dialogue(conversation_id: str, speaker_type: str, message: str):
   
    global dialogues
    # Check if the conversation ID exists, if not, initialize it
    if conversation_id not in dialogues:
        dialogues[conversation_id] = {'conversation_id': conversation_id, 'transcription': []}
    
    # Append the new message to the transcription list for the given conversation ID
    dialogues[conversation_id]['transcription'].append({'speaker': speaker_type, 'message': message})
    mongo_handler.save_Transcription(dialogues[conversation_id],"transcription")
    ...
    class TranscriptionsWorker(AsyncQueueWorker):
        ...
        async def process(self, transcription: Transcription):
            ...
            if transcription.is_final:
                ...
                add_to_dialogue(self.conversation.id, 'human',transcription.message)
    ...
    class SynthesisResultsWorker(InterruptibleAgentResponseWorker):
        ...
        async def process(
            ...
            try:
                ...
                add_to_dialogue(self.conversation.id, 'Ai',message_sent)
                if cut_off:
